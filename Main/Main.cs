...
namespace IDE
{
    public partial class Main : Form
    {
        ...
        private static readonly IniFileEx reConf = Program.reConf;
        ...
        private void OpenFile(object sender, EventArgs e)
        {
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    toolStripStatusLabel2.Text = "Windows NT启动文件(.efi|BOOTMGR)" + text_tsl2;
                    string agreeText = "检测到您正在打开 Windows NT启动文件(.efi|BOOTMGR) 文件，该操作将引起IDE未响应和内存紧张，从而导致程序崩溃，请确认是否继续打开：";
        ...
        private void CheckFileTypeAndAlert(string fileExtension, string tmp)
        {
            ...
            {
                if (func_0a2("exe", "可执行文件(.exe)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("dll"))
            {
                if (func_0a2("dll", "动态链接库(.dll)"))
                    func_0a1(tmp);
            }
            ...
            {
                if (func_0a2("zip|7z|rar|001|tgz|tar|bin", "压缩文件(.zip|.7z|.rar|.001|.tgz|.tar|bin)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("mp3"))
            {
                if (func_0a2("mp3", "媒体文件(.mp3)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("mp4"))
            {
                if (func_0a2("mp4", "视频文件(.mp4)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("crx"))
            {
                if (func_0a2("crx", "Chromium内核浏览器扩展(.crx)"))
                    func_0a1(tmp);
            }
            ...
            {
                if (func_0a2("png|jpg|gif|webp|jpeg|jfif|ico|bmp", "图片文件(.png|.jpg|.gif|.webp|.jpeg|.jfif|.ico|.bmp)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("jar"))
            {
                if (func_0a2("jar", "Java 归档文件|Java 体系结构文件|Minecraft Mod文件(.jar)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("pak") || fileExtension.Equals("pkg"))
            {
                if (func_0a2("pkg", "包文件|ASTRONEER Mod文件(.pak|.pkg)"))
                    func_0a1(tmp);
            }
            ...
            {
                if (func_0a2("iso", "镜像文件(.iso)"))
                    func_0a1(tmp);
            }
            else if (fileExtension.Equals("vmdk"))
            {
                toolStripStatusLabel2.Text = "VMWare 虚拟机虚拟硬盘文件(.vmdk)" + text_tsl2;
                string agreeText = "检测到您正在打开VMWare 虚拟机虚拟磁盘文件(.vmdk)文件，该操作将引起IDE未响应和内存溢出，从而导致程序崩溃且极有可能使您的计算机卡死、崩溃甚至蓝屏，请确认是否继续打开：";
                DialogResult dResult = MessageBoxEX.Show(agreeText, "警告", MessageBoxButtons.YesNo, new string[] { "我已知晓风险", "否" });
                if (dResult != DialogResult.Yes) { return; }
                statusStrip1.Show();
                func_0a1(tmp);
            }
            else if (fileExtension.Equals("sb2"))
            {
            ...
            else if (fileExtension.Equals("sb3"))
            {
