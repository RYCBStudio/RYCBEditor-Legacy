...
namespace IDE;
public partial class XshdFileEditor : UIForm
{
    ...
    public XshdFileEditor(string fileName)
    {
        InitializeComponent();
    ...
    private void CustomSettingsFileEditor_Load(object sender, EventArgs e)
    {
        ...
        if (_fileName.IsNullOrEmpty()) { this.Close(); }
        _editor.Load(_fileName);
        using (Stream s = new FileStream(Main.XshdFilePath + "\\XML.xshd", FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete))
        {
    ...
    private void TextArea_TextEntered(object sender, System.Windows.Input.TextCompositionEventArgs e)
    {
        ...
        FileSavingTip.Text = Main.I18nFile.ReadString("I18n", "text.st.filereadytosave", "text.st.filereadytosave");
        FileSavingIcon.Visible = false;
        FileSavingTip.Visible = false;
        try
        {
            StreamWriter streamWriter = new(_fileName, false, Encoding.UTF8);
            streamWriter.Write(_editor.Text);
            streamWriter.Close();
            FileSavingIcon.Image = Properties.Resources.file_saved_dark;
            FileSavingTip.Text = Main.I18nFile.Localize("text.st.filesaved");
        }
        catch (Exception ex)
        {
            FileSavingIcon.Image = Properties.Resources.file_save_failed_dark;
            FileSavingTip.Text = Main.I18nFile.Localize("text.st.filesavefailed");
