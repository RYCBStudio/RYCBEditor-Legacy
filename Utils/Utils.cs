...
namespace IDE
{
    ...
    public static class Extensions
    {
        ...
        /// </summary>
        /// <returns>如果集合中全是<see langword="true"/>e，则为<see langword="true"/>；否则为<see langword="false"/>。</returns>
    ...
    public static class PythonErrorAnalyzer
    {
        ...
        public static Dictionary<string, object> AnalyzePythonFile(string pythonCode)
        {
            if (!isInitialized)
            {
                throw new Exception("Error has not been initialized. Call Init() method first.");
            }
        ...
        private static void ParseErrors(string pythonCode)
        {
            exceptions.Clear();
            lines.Clear();
            contents.Clear();

            string[] errorLines = _error.Split(Environment.NewLine);
            foreach (string line in errorLines)
            {
                if (line.StartsWith("Traceback") || line.StartsWith("During handling of the above exception, another exception occurred:"))
                {
                    continue;
                }

                Match match = Regex.Match(line, @"^([^:]+):\s(.*?)\s\(detected at line (\d+)\)$");
                if (match.Success)
                {
                    exceptions.Add(match.Groups[1].Value);
                    contents.Add(match.Groups[2].Value);
                    lines.Add(int.Parse(match.Groups[3].Value));
                }
            }
        }
    }
