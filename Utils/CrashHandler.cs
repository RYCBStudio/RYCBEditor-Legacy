...
namespace IDE.Utils
{
    public class CrashHandler
    {
        ...
        private static Dictionary<string, List<string>> _lang_res = new()
        {
        ...
        private static Dictionary<string, List<string>> _port_res = new()
        {
            {"zh-CN", new(){ "客户端", "服务端" } },
            {"zh-TD", new(){ "用戶端", "服務端" } },
        ...
        {
            _ex = ex;
            _path = path + $"\\{_lang_res[GlobalSettings.language][2]}_{DateTime.Now.Year}-{DateTime.Now.Month}-{DateTime.Now.Day}_{DateTime.Now.Hour}-{DateTime.Now.Minute}-{DateTime.Now.Second}+{DateTime.Now.Millisecond}.txt";
        }
        ...
        public void CollectCrashInfo()
        {
            ...
            var InnerExceptionProcess = _ex.InnerException != null ? $"""
                    ...
                    {_lang_res[GlobalSettings.language][1]}: 
                    {_ex.InnerException.StackTrace}
                    """
                : "";
            _resources[GlobalSettings.language] = string.Format(_resources[GlobalSettings.language],
                ...
                _port_res[GlobalSettings.language][0],
        ...
        public bool WriteDumpFile()
        {
            try
            {
                File.WriteAllText(_path, _resources[GlobalSettings.language]);
            }
            catch { return false; }
            return true;
        }
